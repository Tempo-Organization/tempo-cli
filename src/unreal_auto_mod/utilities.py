...
from unreal_auto_mod import file_io, initialization, log, settings, unreal_engine
...
def get_ide_path() -> str:
    return settings.settings_information.settings['optionals']['ide_path']


def get_blender_path():
    return settings.settings_information.settings['optionals']['blender_path']
...
def get_is_using_repak_path_override() -> bool:
    return settings.settings_information.settings['repak_info']['override_default_repak_path']
...
def get_game_info_launch_type_enum_str_value() -> str:
    return settings.settings_information.settings['game_info']['launch_type']


def get_game_id() -> int:
    return settings.settings_information.settings['game_info']['game_id']


def get_game_launcher_exe_path() -> str:
    return settings.settings_information.settings['game_info']['game_launcher_exe']


def get_game_launch_params() -> list:
    return settings.settings_information.settings['game_info']['launch_params']


def get_override_automatic_launcher_exe_finding() -> bool:
    return settings.settings_information.settings['game_info']['override_automatic_launcher_exe_finding']


def get_window_management_events():
    return settings.settings_information.settings["window_management_events"]


def get_engine_launch_args() -> list:
    return settings.settings_information.settings['engine_info']['engine_launch_args']


def get_exec_events() -> list:
    return settings.settings_information.settings['exec_events']


def get_mods_info_list_from_json() -> list:
    return settings.settings_information.settings['mods_info']


def get_game_exe_path() -> str:
    game_exe_path = settings.settings_information.settings['game_info']['game_exe_path']
    file_io.check_path_exists(game_exe_path)
    return game_exe_path


def get_is_using_alt_dir_name() -> bool:
    return settings.settings_information.settings['packaging_uproject_name']['use_override']


def get_alt_packing_dir_name() -> str:
    return settings.settings_information.settings['packaging_uproject_name']['name']
...
def get_override_automatic_version_finding() -> bool:
    return settings.settings_information.settings['engine_info']['override_automatic_version_finding']


def custom_get_unreal_engine_version(engine_path: str) -> str:
    if get_override_automatic_version_finding():
        unreal_engine_major_version = settings.settings_information.settings['engine_info']['unreal_engine_major_version']
        unreal_engine_minor_version = settings.settings_information.settings['engine_info']['unreal_engine_minor_version']
...
def get_unreal_engine_dir() -> str:
    ue_dir = settings.settings_information.settings['engine_info']['unreal_engine_dir']
    file_io.check_path_exists(ue_dir)
    return ue_dir


def get_uproject_file() -> str:
    return settings.settings_information.settings['engine_info']['unreal_project_file']
...
def get_persistent_mods_dir() -> str:
    return f'{settings.settings_information.settings_json_dir}/mod_packaging/persistent_files'
...
def get_persistant_mod_dir(mod_name: str) -> str:
    return f'{settings.settings_information.settings_json_dir}/mod_packaging/persistent_files/{mod_name}'
...
def get_is_overriding_default_working_dir() -> bool:
    return settings.settings_information.settings['general_info']['override_default_working_dir']


def get_override_working_dir() -> str:
    return settings.settings_information.settings['general_info']['working_dir']
...
def get_engine_cooking_args() -> list:
    return settings.settings_information.settings['engine_info']['engine_cooking_args']


def get_engine_packaging_args() -> list:
    return settings.settings_information.settings['engine_info']['engine_packaging_args']


def get_engine_building_args() -> list:
    return settings.settings_information.settings['engine_info']['engine_building_args']


def get_is_overriding_automatic_version_finding() -> bool:
    return settings.settings_information.settings['repak_info']['override_automatic_version_finding']


def get_override_automatic_window_title_finding() -> bool:
    return settings.settings_information.settings['game_info']['override_automatic_window_title_finding']


def get_window_title_override() -> str:
    return settings.settings_information.settings['game_info']['window_title_override']


def get_window_title() -> str:
    return settings.settings_information.settings['general_info']['window_title']
...
def get_cleanup_repo_path() -> str:
    return settings.settings_information.settings['git_info']['repo_path']


def get_unreal_engine_building_main_command() -> str:
    return settings.settings_information.settings['engine_info']['engine_building_command']


def get_unreal_engine_cooking_main_command() -> str:
    return settings.settings_information.settings['engine_info']['engine_cooking_command']


def get_unreal_engine_packaging_main_command() -> str:
    return settings.settings_information.settings['engine_info']['engine_packaging_command']
