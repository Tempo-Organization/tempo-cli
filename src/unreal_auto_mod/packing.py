...
from unreal_auto_mod import app_runner, data_structures, file_io, hook_states, logger, settings, utilities
...
def uninstall_pak_mod(mod_name: str):
    extensions = unreal_engine.get_game_pak_folder_archives(settings.get_uproject_file(), utilities.custom_get_game_dir())
...
def cooking():
    populate_queue()
    if unreal_engine.get_is_game_iostore(settings.get_uproject_file(), utilities.custom_get_game_dir()):
