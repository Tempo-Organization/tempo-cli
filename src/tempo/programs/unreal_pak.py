...
import tempo.app_runner
import tempo.settings
from tempo import file_io, packing, utilities
from tempo.data_structures import CompressionType
from tempo.programs import unreal_engine


def get_pak_dir_to_pack(mod_name: str):
    return f"{tempo.settings.get_working_dir()}/{mod_name}"


def make_response_file_iostore(mod_name: str) -> str:
    file_list_path = os.path.join(
        tempo.settings.get_working_dir(), f"{mod_name}_filelist.txt"
    )
...
def make_ue4_iostore_mod(mod_name: str, final_pak_file: str):
    unreal_engine_dir = tempo.settings.get_unreal_engine_dir()
    ...
    uproject_name = os.path.splitext(
        os.path.basename(tempo.settings.get_uproject_file())
    )[0]
    global_utoc_path = f"{utilities.get_uproject_dir()}/Saved/StagedBuilds/{ue_win_dir_str}/{uproject_name}/Content/Paks/global.utoc"
    cooked_content_dir = f"{tempo.settings.get_working_dir()}/{mod_name}"

    commands_txt_content = get_iostore_commands_file_contents(mod_name, final_pak_file)
    commands_txt_path = f"{tempo.settings.get_working_dir()}/iostore_packaging/{mod_name}_commands_list.txt"
    ...
    platform_string = unreal_engine.get_win_dir_str(
        tempo.settings.get_unreal_engine_dir()
    )
    iostore_txt_location = f"{tempo.settings.get_working_dir()}/iostore_packaging/{mod_name}_iostore.txt"
    # default_engine_patch_padding_alignment = 2048
    args = [
        # unreal_pak,
        f'"{tempo.settings.get_uproject_file()}"',
        "-run=IoStore",
    ...
    tempo.app_runner.run_app(exe_path=exe, args=args)
...
def make_iostore_unreal_pak_mod(
    ...
    if unreal_engine.is_game_ue4(tempo.settings.get_unreal_engine_dir()):
...
def make_non_iostore_unreal_pak_mod(
    ...
    *,
    ...
    tempo.app_runner.run_app(command)
...
def install_unreal_pak_mod(
    ...
    output_pak_dir = f"{tempo.settings.get_working_dir()}/{utilities.get_pak_dir_structure(mod_name)}"
    intermediate_pak_file = f"{tempo.settings.get_working_dir()}/{utilities.get_pak_dir_structure(mod_name)}/{mod_name}.pak"
    ...
    is_game_iostore = unreal_engine.get_is_game_iostore(
        tempo.settings.get_uproject_file(), utilities.custom_get_game_dir()
    )
